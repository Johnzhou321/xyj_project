...
public class DateUtils extends org.apache.commons.lang.time.DateUtils {
	 ...
	 * 比较日期的模式 --比较月份
	 */
	public final static int COMP_MODEL_MONTH = 5;
	...
	static {
		...
		if (timeUnit == null) {
			...
			timeUnit.put("second",Calendar.SECOND);
	 ...
	 * @return
	 */
	public static String getData(int addDate, String dateFormat) {
	...
	public static long daysBetween(Date fromDate, Date toDate) {
		return (toDate.getTime() - fromDate.getTime()) / DAY_MILLI;
	}

	public static long monthsBetween(Date fromDate, Date toDate) {
		int yearDiff = getYearDiff(fromDate, toDate);
		...
		return (toYear-fromYear+1)*12-fromMonth-(12-toMonth);
	}
	...
	public static boolean isBetween(Date now, Date start, Date end, int model) {
		return isBetween(now, start, end, model, COMP_MODEL_DATETIME);
	}
	...
	public static String getBackDate(int interval) {
		Calendar cal = Calendar.getInstance();
		cal.add(Calendar.DATE, -1 * interval);
		return formatDate(cal.getTime(), "yyyy-MM-dd");
	}

	public static String getBackDate(String date, int interval) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(DateUtils.getDateFromString(date, "yyyy-MM-dd"));
		cal.add(Calendar.DATE, -1 * interval);
		return formatDate(cal.getTime(), "yyyy-MM-dd");
	}
	
	public static String getForwardDate(String date, int interval) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(DateUtils.getDateFromString(date, "yyyy-MM-dd"));
	 ...
	 * @return
	 */
	public static Date getDateByStrSpecial(String dateStr, String Type) {
		if ("A".equals(Type))
	...
	/**
	 * 对yyyyMMddhhmmss时间进行格式化yyyy-MM-dd hhmmss
	 * 
	 * @param dateString
	 * @return
	 */
	public static String getDateString(String date) {
		...
		try {
			...
			String dateStr = sdfs.format(newDate);
			return dateStr;
		} catch (ParseException e) {
			// 做转化失败的操作
			return null;
	 ...
	 * @return
	 */
	public static String getDateBeforeMonth(Date date, int month) {
		Calendar c = Calendar.getInstance();
		Date formatDate = null;
		String[] arr = formatDate(date, "yyyy-MM-dd HH:mm:ss").split(" ");
		try {
			formatDate = SHORT_DATE_FORMAT.parse(arr[0]);
		} catch (Exception e) {
		}
		c.setTime(formatDate);
		c.add(Calendar.MONTH, month);
		return SHORT_DATE_FORMAT.format(c.getTime()) + " " + arr[1];
	}
	 ...
	 * @param dateStr1
	 * @param dateStr2
	 * @return返回格式yyyyMMddHHmmss
	 * @throws ParseException
	 */
	public static Date getMonthBeforeSameDate(int month) {
		Date date = new Date();
		Calendar calendar = Calendar.getInstance();
	...
	public static void main(String[] args) {
		String ddd = "";
		System.out.println(getMonthBeforeSameDate(3));
		System.out.println(compareDate(new Date(), getMonthBeforeSameDate(3), 5));
