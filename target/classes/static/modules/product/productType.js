...
function operateFormatter(value,row,index) {
    var text='';
    text+='<a style="margin: 3px;" href="#" class="btn btn-white btn-sm" onclick="addForm(\''+row.id+'\',\'编辑分类\');" title="编辑"><i class="fa fa-pencil"></i> 编辑</a>';
    text+='<a style="margin: 3px;" href="#" class="btn btn-white btn-sm" onclick="deleteType(\''+row.id+'\');" title="删除"><i class="glyphicon glyphicon-trash"></i> 删除</a>';
    return text;
}
...
function searchListener(){
    ...
    $("#type_list_table").bootstrapTable('selectPage',1);  //跳转到第一页，防止其他页查询第一次不显示数据的问题。
    $("#type_list_table").bootstrapTable('refresh');       //无参数刷新
}
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.resultStat=="SUCCESS") {
                        ...
                        layer.close(index);
                        $('#type_list_table').bootstrapTable('refresh');
                    } else {
                        layer.alert("删除失败");
                    }
                },
...
function batchDeleteRole() {
    var data=$("#type_list_table").bootstrapTable('getSelections');
    var ids=[];
...
function addForm(id,type) {
    typeFormLayer= layer.open({
        ...
        closeBtn: 1,
        shadeClose: true,
        content: '/productType/typeForm',
        end: function () {
            $('#type_list_table').bootstrapTable('refresh');
        },
        success: function(layero, index){
            var body=layer.getChildFrame('body',index);
            var $form=$(body).find("#type_form");
            if(type!="新增分类"){
               /* if(type=="查看用户"){
